...
public class ClientProxy extends CommonProxy {
	...
	public void initMod() {
		...
		RenderingRegistry.registerEntityRenderingHandler(
				EntityMorphAcquisition.class,
				Morph.proxy.tickHandlerClient.renderMorphInstance);
	}
	...
	public void initPostMod() {
		...
		try {
			List entityRenderers = (List) ObfuscationReflectionHelper
					.getPrivateValue(RenderingRegistry.class,
							RenderingRegistry.instance(), "entityRenderers");
		...
		for (int i = compatibleEntities.size() - 1; i >= 0; i--) {
			...
			Render rend = EntityHelper
					.getEntityClassRenderObject(compatibleEntities.get(i));
		...
		for (Class clz : compatibleEntities) {
			try {
				RendererLivingEntity rend = (RendererLivingEntity) renders
						.get(clz);
				ModelList.addModelInfo(
						clz,
						new ModelInfo(clz, rend,
								(ModelBase) ObfuscationReflectionHelper
										.getPrivateValue(
												RendererLivingEntity.class,
												rend, ObfHelper.mainModel)));
			} catch (Exception e) {
